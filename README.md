...
1. `.env`ファイルを作成し、必要な環境変数を設定（`.env.example`を参照）
2. 取得したクライアントIDを `.env` の `GOOGLE_CLIENT_ID` に設定（フロントエンドは起動時に `GET /api/config` から実行時取得します。HTMLやJSへのハードコードは不要です）
...
| URL | 用途 | トークン |
|-----|------|----------|
| `GET /api/ical/all?token=<token>` | 全スタッフのシフト（管理者用） | `.env` の `ICAL_TOKEN` |
| `GET /api/ical/user/{user_id}?token=<token>` | 特定ユーザーのシフト | ユーザーごとの導出トークン |
| `GET /api/ical/my-url?userId=<user_id>` | 本人の購読URLを発行 | 不要 |
| `GET /api/ical/all-url?email=<email>` | 管理者用の購読URLを発行 | `AUTHORIZED_EMAILS` に含まれること |

### トークン設計

`ICAL_TOKEN` はサーバー秘密鍵として扱い、フロントエンドには配布しません。各ユーザーの購読トークンは `HMAC-SHA256(ICAL_TOKEN, "ical-user:" + user_id)` で導出され、**その user_id と対でのみ有効**です。

- 購読URLの `user_id` を他人のものに書き換えても `401` になります
- 管理者トークンで `/api/ical/user/{user_id}` を叩いても `401` になります（用途を分離）
- `ICAL_TOKEN` を再生成すると、全ユーザーの購読URLが一括で失効します

### 購読URLの確認

- **管理者**: 「シフト一覧」タブ上部に購読URL（全シフト）が表示されます
- **一般ユーザー**: 「自分のシフト一覧」タブ上部に、自分のシフトだけの購読URLが表示されます

いずれも「コピー」ボタンでクリップボードにコピーできます。学生向けの詳しい手順は `help.html` の「Googleカレンダーに表示する」を参照してください。
...
- **カレンダー名**: 購読するとカレンダーアプリ上に、個人用は「〇〇さんのシフト」（シフトが0件の場合は「マイシフト」）、全体用は「全員のシフト」という名前で追加されます。複数のカレンダーを購読しても区別できます。
- **セキュリティ**: URLに含まれるトークンが認証情報です。購読URLは他人に共有しないでください。漏洩した場合は `.env` の `ICAL_TOKEN` を再生成してサーバーを再起動すると、全ユーザーの購読URLが失効し、新しいURLが発行されます。
...
### データバックアップ
- データはSQLiteファイル（`data/shift.db`、`.env` の `DATABASE_PATH` で変更可）に保存されます
- 定期的なバックアップを推奨
